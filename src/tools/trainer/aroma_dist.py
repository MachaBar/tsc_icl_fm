...
from src.modules.aroma import AROMAEncoderDecoderICL, FourierEncoderDecoderICL
...
def prepare_optim(
    ...
    use_target_for_training = True if is_icl_training else getattr(cfg_optim, 'use_target_for_training', True)
...
def train_fn(
    ...
    is_icl_training = (isinstance(inr.module, AROMAEncoderDecoderICL) or isinstance(inr.module, FourierEncoderDecoderICL)) if isinstance(inr, DDP) else (isinstance(inr, AROMAEncoderDecoderICL) or isinstance(inr, FourierEncoderDecoderICL))
    # assert is_icl_training

    # Define optimizer:
    optimizer = get_optimizer(inr, cfg_optim, rank=rank)

    # gradient accumulation:
    grad_accum_steps = getattr(cfg_optim, 'grad_accum_steps', 1)
    OmegaConf.update(cfg_optim.scheduler, 'T_max', int(cfg_optim.scheduler.T_max / grad_accum_steps))

    if (rank == 0):
        logger.info('[Training] Distributed training: {} GPUs x {} gradient accumulation steps'.format(dist.get_world_size(), grad_accum_steps))
    ...
    # get asinh transform:
    apply_asinh_transform = inr.module.apply_asinh_transform
    ...
    # AMP grad scaler:
    use_amp = getattr(cfg_optim, 'use_amp', False)
    amp_scaler = torch.amp.GradScaler(device.type) # pyright: ignore[reportPrivateImportUsage]

    global_steps = 0

    if rank == 0:
        logger.info('[Training] Max number of epochs {:,d} / steps {:,d}'.format(max_epochs, 0 if max_global_steps==np.inf else max_global_steps))
        logger.info('[Training] Using Mixed Precision: {}'.format(use_amp))
    ...
    for step in range(max_epochs):
        ...
        if isinstance(train_loader.batch_sampler, DistributedJointSampler):
        ...
        fit_loss_epoch = 0
        metrics_epoch = {
            '{}_{}'.format(metric_name, segment): 0.
        ...
        for substep, batch in enumerate(train_loader):
            ...
            # # preallocating memory might help:
            # # https://docs.pytorch.org/tutorials/recipes/recipes/tuning_guide.html#preallocate-memory-in-case-of-variable-input-length
            # preallocate_step = (step == 0) and (substep == 0)
            # if preallocate_step:
            #     series_c = torch.randn(len(series_c), int(cfg.model.max_context_len), 1).to(device)
            #     coords_c = torch.randn(len(coords_c), int(cfg.model.max_context_len), 1).to(device)
            #     covar_c  = torch.randn(len(coords_c), 6, int(cfg.model.max_context_len), 1).to(device)
            #     if cfg.task.name == 'forecasting':
            #         len_t = int(cfg.model.max_target_len)
            #     else:
            #         len_t = int(0.1 * cfg.model.max_context_len)
            #     coords_t = torch.randn(len(coords_t), len_t, 1).to(device)
            #     series_t = torch.randn(len(coords_t), len_t, 1).to(device)
            #     covar_t  = torch.randn(len(coords_c), 6, len_t, 1).to(device)                
            ...
            #     #   'obs_ratio={:.2f}'.format(series_c.shape[1]/(series_c.shape[1]+series_t.shape[1])),
            #         'horizon len={}'.format(series_t.shape[1]),
            #       'ncovars={}, covar_len={}'.format(covar_all.shape[1],covar_all.shape[2]) if (covar_c is not None) else None,
            #       )

            n_samples = series_c.shape[0]
            if n_samples == 0:
                print('{} nsamples is 0, skip step {}_{}'.format(rank, step, substep))
                continue
            
            # build query coords:
            query_coords = torch.cat([coords_c, coords_t], dim=1) # [N, T_in+T_mis, 1]

            # check if we do grad accum or not:
            is_last_micro_step = ((step + 1) % grad_accum_steps == 0) or (step + 1 == len(train_loader))
            ...
            ctx = inr.no_sync() if not is_last_micro_step else nullcontext()
            ...
            with ctx:
                ...
                with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=use_amp):
                    out, _ = inr(
                        series               = series_c,
                        coords               = coords_c,
                        target_coords        = query_coords if is_icl_training else coords_t,
                    ...
                    # loss on target coords:
                    out = out.type(torch.float32)
                    fit_loss: torch.Tensor = loss_fn(out, transform(series_t))#.mean()

                    if fit_loss.isnan().any():
                        # ne sert à rien car ok la loss est 0 mais des grad intermédiaires sont nans
                        fit_loss = torch.nan_to_num(fit_loss.nanmean(), nan=0.0)
                        
                        print('{} loss is nan, skip step {}_{}, target is Nan: {}+{}, context is Nan: {}, coords_c is Nan: {}, query coords is Nan: {}, covar is Nan: {}'.format(
                            rank, step, substep, torch.isnan(series_t).any(), torch.isnan(transform(series_t)).any(),
                            torch.isnan(series_c).any(),
                            torch.isnan(coords_c).any(),
                            torch.isnan(query_coords).any(),
                            torch.isnan(covar_all).any() if covar_all is not None else None
                            )
                        )
                        continue
                    else:
                        fit_loss = fit_loss.mean()

                    # get total train loss:
                    loss = fit_loss / grad_accum_steps
            ...
            for metric_name in metric_to_monitor:
                per_step_callback, metrics_epoch = update_metrics(
                    yhat              = inv_transform(out[..., median_idx:median_idx+1]),
                    series            = series_t,
                    metric_epoch      = metrics_epoch,
                    per_step_callback = per_step_callback,
                    key               = '{}_{}'.format(metric_name, 'target')
                )

            # store step losses:
            per_step_callback['loss'].append(loss.item() * grad_accum_steps)
        ...
        if rank == 0:
            ...
            # save best model:
            if (fit_loss_epoch > 0) and (fit_loss_epoch < best_loss):
